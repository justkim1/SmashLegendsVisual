...
struct RenderConfig {
    ...
    // Smash Legends visual-only helpers (no input injection)
    std::atomic<bool> smashVisualMode{true};
    std::atomic<bool> smashDistanceLine{true};
    std::atomic<bool> smashRangeCircle{false};
    std::atomic<float> smashRangeRadius{140.0f};
