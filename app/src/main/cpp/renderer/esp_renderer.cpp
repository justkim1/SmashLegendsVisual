...
void ESPRenderer::drawESPBoxes(const DetectionResult& result) {
    ...
    // Smash Legends visual-only pairing:
    // Treat the detection closest to screen center as the local-player candidate,
    // then choose the nearest different detection as the opponent candidate.
    // This is a temporary heuristic until a Smash-specific model can distinguish
    // self/enemy classes. It never injects touch/key input.
    int selfCandidateIdx = -1;
    int opponentCandidateIdx = -1;
    float selfCandidateDistSq = 1.0e30f;
    ...
    auto drawBox = [&](const BoundingBox& box, int index) {
        ...
        // Local-player candidate = detection nearest screen center.
        float distSq = (boxCenterX - centerX) * (boxCenterX - centerX) +
                       (boxCenterY - centerY) * (boxCenterY - centerY);
        if (distSq < selfCandidateDistSq) {
            selfCandidateDistSq = distSq;
            selfCandidateIdx = index;
        }
    ...
    // Smash Legends visual-only distance line/range.
    if (g_settings.smashVisualMode && selfCandidateIdx >= 0) {
        const BoundingBox& selfBox = result.boxes[selfCandidateIdx];
        const ImVec2 selfPoint(selfBox.x + selfBox.width * 0.5f,
                               selfBox.y + selfBox.height * 0.5f);

        float nearestOpponentSq = 1.0e30f;
        for (int i = 0; i < count; ++i) {
            if (i == selfCandidateIdx) continue;
            const BoundingBox& candidate = result.boxes[i];
            if (candidate.confidence < threshold || candidate.width <= 1.0f || candidate.height <= 1.0f) continue;
            const float cx = candidate.x + candidate.width * 0.5f;
            const float cy = candidate.y + candidate.height * 0.5f;
            const float dx = cx - selfPoint.x;
            const float dy = cy - selfPoint.y;
            const float d2 = dx * dx + dy * dy;
            if (d2 < nearestOpponentSq) {
                nearestOpponentSq = d2;
                opponentCandidateIdx = i;
            }
        }

        if (opponentCandidateIdx >= 0) {
            const BoundingBox& opponent = result.boxes[opponentCandidateIdx];
            const ImVec2 opponentPoint(opponent.x + opponent.width * 0.5f,
                                       opponent.y + opponent.height * 0.5f);
            const float distancePx = std::sqrt(nearestOpponentSq);

            if (g_settings.smashDistanceLine) {
                drawList->AddLine(selfPoint, opponentPoint, IM_COL32(255, 220, 60, 240), 2.5f);
                char distanceText[48];
                snprintf(distanceText, sizeof(distanceText), "DIST %.0f px", distancePx);
                const ImVec2 mid((selfPoint.x + opponentPoint.x) * 0.5f,
                                 (selfPoint.y + opponentPoint.y) * 0.5f);
                drawList->AddText(ImVec2(mid.x + 2.0f, mid.y + 2.0f), IM_COL32(0,0,0,200), distanceText);
                drawList->AddText(mid, IM_COL32(255, 255, 255, 255), distanceText);
            }

            if (g_settings.smashRangeCircle) {
                drawList->AddCircle(opponentPoint, g_settings.smashRangeRadius,
                                    IM_COL32(255, 180, 40, 190), 48, 2.0f);
            }
        }
    }
}
...
void ESPRenderer::drawMenu() {
    ...
    if (ImGui::Begin("##ESP_Settings", nullptr, windowFlags)) {
        ...
        ImGui::Separator();
        ImGui::TextUnformatted("Smash Legends Visual Mode");
        ImGui::TextUnformatted("Visual only - no touch/key injection");
        if (ImGui::Checkbox("Smash Visual Mode", &g_settings.smashVisualMode)) {}
        if (ImGui::Checkbox("Distance Line", &g_settings.smashDistanceLine)) {}
        if (ImGui::Checkbox("Range Circle", &g_settings.smashRangeCircle)) {}
        if (g_settings.smashRangeCircle) {
            float radius = g_settings.smashRangeRadius;
            if (ImGui::SliderFloat("Range Radius", &radius, 30.0f, 400.0f, "%.0f px")) {
                g_settings.smashRangeRadius = radius;
            }
        }
